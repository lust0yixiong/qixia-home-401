import {FINISHES} from './surface-finishes.js?v=20';
...
export function validatePlan(plan) {
  if (!plan || ![1, 2, 3].includes(plan.version) || !plan.materials || typeof plan.materials !== 'object' || Array.isArray(plan.materials)) throw Error('不是支持的材质方案文件。');
  const result = {};
  for (const [key, c] of Object.entries(plan.materials)) {
    if (!GROUPS.some(([id]) => id === key)) throw Error('方案包含未知材质。');
    if (!c || !/^#[0-9a-f]{6}$/i.test(c.color) || !['original', 'solid', 'custom', 'preset'].includes(c.mode)) throw Error('方案中的颜色或贴图类型无效。');
    ...
    if (c.mode === 'preset' && !FINISHES.some(p=>p.id===c.preset)) throw Error('未知的材质预设。');
    if (c.metalness !== undefined && (!Number.isFinite(c.metalness)||c.metalness<0||c.metalness>1)) throw Error('金属感参数无效。');
    ...
    result[key] = {color: c.color.toLowerCase(), mode: c.mode, repeatX: c.repeatX, repeatY: c.repeatY, rotation: c.rotation, roughness: c.roughness,
      ...(c.metalness !== undefined ? {metalness:c.metalness} : {}), ...(c.mode === 'preset' ? {preset:c.preset} : {}),
...
export function initMaterialEditor({THREE, materials, renderer, scene, camera, finishLibrary, onOpen}) {
  ...
  for (const [key] of GROUPS) {
    ...
    defaults[key] = {color: `#${m.color.getHexString()}`, mode: 'original', repeatX: m.map?.repeat.x || 1, repeatY: m.map?.repeat.y || 1, rotation: 0, roughness: m.roughness, metalness:m.metalness, map: m.map};
  ...
  function sync() {
    ...
    for (const field of ['repeat-x', 'repeat-y', 'roughness', 'metalness']) {
      const prop = {'repeat-x': 'repeatX', 'repeat-y': 'repeatY', roughness: 'roughness',metalness:'metalness'}[field];
      $(`material-${field}`).value = c[prop]??defaults[base][prop];$(`material-${field}-value`).value = String(c[prop]??defaults[base][prop]);
    }
    $('material-rotation').value = c.rotation;
    const currentMaterial=materialFor(active),hasMap = !!currentMaterial.map;
    const hasDetail=hasMap||!!currentMaterial.bumpMap||!!currentMaterial.normalMap;
    $('material-preset').value=c.mode==='preset'?c.preset:'';
  ...
  for (const [id, prop] of [['repeat-x','repeatX'],['repeat-y','repeatY'],['rotation','rotation'],['roughness','roughness'],['metalness','metalness']]) {
  ...
  $('material-preset').onchange=()=>{
    const p=FINISHES.find(p=>p.id===$('material-preset').value);if(!p){sync();return;}
    ensureEditable();settings[active]={mode:'preset',preset:p.id,color:p.color,roughness:p.roughness,metalness:p.metalness,repeatX:1,repeatY:1,rotation:0};
    replaceMap(active,finishLibrary.get(p.id).map?.clone()||null);apply(active);sync();save();
  };
  ...
  async function loadPlan(plan) {
    ...
    try {
      for (const [key,c,base] of entries) {
        ...
        } else if(c?.mode==='preset'){const map=finishLibrary.get(c.preset).map;if(map)prepared.set(key,map.clone());}
