...
const drawings=[
 ['plan','P.01 · 平面布置'],['dimensions','P.12 · 家具尺寸'],['flooring','P.08 · 地面铺贴'],
...
function texture(kind){let c=document.createElement('canvas');c.width=c.height=512;let ctx=c.getContext('2d'),seed=39;const rand=()=>{seed=(1664525*seed+1013904223)>>>0;return seed/4294967296};ctx.fillStyle=kind==='wood'?'#b99570':'#d4cfc2';ctx.fillRect(0,0,512,512);if(kind==='wood'){for(let i=0;i<950;i++){ctx.strokeStyle=`rgba(${rand()>.5?'82,51,28':'235,212,177'},${rand()*.09})`;let x=rand()*512;ctx.beginPath();ctx.moveTo(x,0);ctx.bezierCurveTo(x+rand()*10,170,x-rand()*10,340,x+rand()*6,512);ctx.stroke()}}else{let id=ctx.getImageData(0,0,512,512);for(let j=0;j<id.data.length;j+=4){let v=(rand()-.5)*18;id.data[j]+=v;id.data[j+1]+=v;id.data[j+2]+=v}ctx.putImageData(id,0,0);for(let i=0;i<70;i++){ctx.strokeStyle='rgba(93,90,78,.05)';ctx.beginPath();let x=rand()*512,z=rand()*512;ctx.moveTo(x,z);ctx.lineTo(x+rand()*100,z+rand()*20);ctx.stroke()}}let t=new THREE.CanvasTexture(c);t.colorSpace=THREE.SRGBColorSpace;t.wrapS=t.wrapT=THREE.RepeatWrapping;t.repeat.set(kind==='wood'?1:5,kind==='wood'?1:5);return t}
...
function resize(){if(!renderer||mode!=='scene')return;let b=$('#canvas-wrap').getBoundingClientRect();if(!b.width||!b.height)return;camera.aspect=b.width/b.height;camera.updateProjectionMatrix();renderer.setSize(b.width,b.height);}
...
function animate(){requestAnimationFrame(animate);if(mode!=='scene')return;if(pulloutFlight){const t=reduced?1:Math.min((performance.now()-pulloutFlight.start)/700,1),v=t*t*(3-2*t);pulloutGroup.position.x=THREE.MathUtils.lerp(pulloutFlight.from,pulloutFlight.to,v);if(t===1)pulloutFlight=null}if(flight){let t=Math.min((performance.now()-flight.start)/950,1),v=1-Math.pow(1-t,3);camera.position.lerpVectors(flight.from,flight.to,v);controls.target.lerpVectors(flight.fromTarget,flight.toTarget,v);if(t===1)flight=null}controls.update();renderer.render(scene,camera);if(frame++%2===0){let w=renderer.domElement.clientWidth,h=renderer.domElement.clientHeight;for(let l of labelItems){let p=l.point.clone().project(camera);l.el.style.left=(p.x*.5+.5)*w+'px';l.el.style.top=(-p.y*.5+.5)*h+'px';l.el.style.opacity=showLabels&&selected==='all'&&p.z<1&&p.z>0&&p.x>-1&&p.x<1&&p.y>-1&&p.y<1?'1':'0'}}}
...
selectRoom('all',false);try{build()}catch(err){console.error(err);$('#loading').textContent='三维画面暂时无法加载，请使用支持 WebGL 的浏览器。仍可查看设计效果图和平面图。';document.querySelectorAll('.scene-tools button,.view-controls button,.zoom-controls button').forEach(b=>b.disabled=true)}
